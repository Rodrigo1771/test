...
data_dir = 'train+test/'
BIO = { "beginning": 'B-PROCEDIMIENTO', "inside": 'I-PROCEDIMIENTO', "outside": 'O' }
...
for i in range(0, len(files), 2):
    ...
    for word in words:
        if txt_file == 'es-S0212-71992000001200006-1.txt' and word == ' positivo.VIH':
            ...
            new_words.append(word[:-2])
            new_words.append(word[-2:])
        elif txt_file == 'es-S0004-06142010000500014-1.txt' and word == ' Ampicilina,Amoxicilina':
            new_words.append(' Ampicilina')
            new_words.append(',')
            new_words.append('Amoxicilina')
        elif txt_file == 'es-S0004-06142010000500014-1.txt' and word == ' Eritromicina,Tetraciclina':
            new_words.append(' Eritromicina')
            new_words.append(',')
            new_words.append('Tetraciclina')
        elif txt_file == 'es-S0213-12852006000600002-1.txt' and word == ' ·3TC':
            new_words.append(' ·')
            new_words.append('3TC')
        elif txt_file == 'es-S0376-78922012000200008-1.txt' and word == ' adriamicinaciclofosfamida':
...
with open(f'out/train-full.conll', 'w') as f:
    for i, str in enumerate(final_strings):
        if i in [
            ...
            305379, 343385, 345977, 347160, 347162, 347164, 366796, 395299, 406158, 406187, 424138, 7074, 15411,
            22385, 28863, 39445, 39448, 39698, 39701, 39704, 39707, 39916, 71710, 71926, 140773, 175022, 176145,
            207022, 207036, 207418, 207422, 212702, 212714, 212739, 212790, 213492, 217930, 222539, 223873, 230514,
            232456, 248330, 249458, 256874, 264132, 268924, 270182, 299097, 299728, 302507, 325877, 389185, 389604,
            len(final_strings)-1
        ]:
            continue
        f.write(str)

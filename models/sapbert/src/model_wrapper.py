...
class Model_Wrapper(object):
    ...
    def embed_dense(self, names, show_progress=False, batch_size=4096, agg_mode="cls", description=None):
        """
        ...
        with torch.no_grad():
            if show_progress:
                iterations = tqdm(range(0, len(names), batch_size), desc=description)
            else:
